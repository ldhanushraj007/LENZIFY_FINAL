...
export async function getCart() {
  try {
    ...
    const { data, error } = await supabase
      .from("cart")
      .select("*, products(id, name, brand, price, offer_price, discount_price, stock, frame_type, product_type, specifications, primary_image, product_images(*), categories(id, name, slug))")
      .eq("user_id", user.id)
